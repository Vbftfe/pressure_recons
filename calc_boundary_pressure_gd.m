function [boundary_pressure_gd_x, boundary_pressure_gd_y] = calc_boundary_pressure_gd(ux, uy, domain_boundary, miu, Lx, Ly)
dux_dx_2 = zeros(1, length(ux));
dux_dy_2 = dux_dx_2;
duy_dx_2 = dux_dx_2;
duy_dy_2 = dux_dx_2;
boundary_pressure_gd_x = dux_dx_2;
boundary_pressure_gd_y = dux_dx_2;
...
for i = 1:length(ux)
    if domain_boundary(i)
        ...
        else % 除上下边界外，计算边界上关于y方向的导�?
            % 对边界为曲面的位置进行特殊处�?
            if mod(i - 1, Lx) < Lx / 2
                if isnan(ux(i - Lx))
                    idx = find(~isnan(ux((i - Lx):end)), 1);
        ...
        %         %% 计算边界的压力�??
        %         % 使用x方向的梯度分量计算左右边�?
        %         if mod(i - 1, Lx) < Lx / 2   % 计算左边�?
        %             boundary_pressure_gd_x(i) = miu*(dux_dx_2(i) + dux_dy_2(i));
        %         end
        %
        %         if mod(i - 1, Lx) > Lx / 2   % 计算右边�?
        %             boundary_pressure_gd(i) = miu*(dux_dx_2(i) + dux_dy_2(i));
        %         end
        %
        %         % 使用y方向的梯度分量计算下边界
        %         if floor((i - 1) / Lx) == Ly - 1 && (mod(i, Lx) ~= 1 &&  mod(i, Lx) ~= 0)
        %             boundary_pressure_gd(i) = miu*(duy_dx_2(i) + duy_dy_2(i));
        %         end
        %
        %         % 使用y方向的梯度分量计算上边界
        ...
        %% 计算边界处的梯度
        boundary_pressure_gd_x(i) = miu*(dux_dx_2(i) + dux_dy_2(i));
        boundary_pressure_gd_y(i) = miu*(duy_dx_2(i) + duy_dy_2(i));
    end
end
% % 将左上角和右上角置零
